---
id: metamodel-governance.application-service
protocol: Java Interface
version: 1.0.0
owner: metamodel-governance
description: 提供元模型治理 BC 对外暴露的 Application Service 接口契约，下游 BC 通过 Maven 依赖 metaforge-metamodel-api 模块后可直接调用，覆盖 Bundle 生命周期、版本治理、元素定义 CRUD、Package 命名空间、导出清单、依赖治理、声明式导入导出及校验能力。
type: business
---

# Application Service Contract: metamodel-governance

**Protocol**: Application Service（进程内 Java Interface 调用）
**Module**: `metaforge-metamodel-api`
**Version**: 1.0.0

> 下游 BC（metadata-management、semantic-query-engine、semantic-relation-network、agent-consumption）通过 Maven 依赖 `metaforge-metamodel-api` 模块，注入以下接口的 Spring Bean 实例进行进程内调用。
...
<dependency>
    ...
    <artifactId>metaforge-metamodel-api</artifactId>
