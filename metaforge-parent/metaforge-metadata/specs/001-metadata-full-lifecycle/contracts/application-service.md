...
## 6. MetadataEventService

**职责**: 变更事件通知——元数据生效/下线时发布变更事件，通知下游 BC 同步数据。

```java
/**
 * 元数据变更事件服务（内部接口，下游 BC 通过 Spring Event 监听）。
 * <p>
 * 生效/下线事务成功提交后自动发布变更事件，驱动下游语义关系网络、
 * 查询引擎等模块同步数据。
 */
public interface MetadataEventService {
     ...
     * 发布元数据变更事件。
     * <p>
     * 事件内容包含 FQN、操作类型（ACTIVATE/DEPRECATE）、版本号、时间戳。
     * 通过 Spring ApplicationEventPublisher 发布，下游通过 @EventListener 或
     * @TransactionalEventListener(phase = AFTER_COMMIT) 接收。
     *
     * @param fqn 元数据 FQN
     * @param changeType 变更类型（ACTIVATE/DEPRECATE）
     * @param version 版本号
     */
    void publishChangeEvent(String fqn, ChangeType changeType, int version);
}
```
