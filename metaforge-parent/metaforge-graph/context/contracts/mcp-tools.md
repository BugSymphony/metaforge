---
id: semantic-relation-network.mcp-tools
protocol: MCP (Model Context Protocol)
version: 1.0.0
owner: semantic-relation-network
description: 提供语义关系网络 BC 通过 Spring AI 暴露的 MCP 工具集契约，覆盖关系精准查询、出入边查询、多维过滤、拓扑查询与历史版本追溯等语义能力，由 agent-consumption BC 统一聚合发布。
type: business
---

# MCP Tools Contract: semantic-relation-network

**Protocol**: MCP (Model Context Protocol, Spring AI)
**Publisher**: `agent-consumption`（统一代理发布方）/ 本 BC（工具定义方）
**Version**: 1.0.0

> **注意**: 全局架构约束要求 MCP Server 由 `agent-consumption` BC 统一发布。本 BC 声明工具语义与签名，最终由 `agent-consumption` BC 通过 Spring AI 聚合发布为 MCP 工具集。本契约定义的工具即为对外承诺的 MCP 能力边界。
