---
id: semantic-relation-network.rest-api
protocol: REST
version: 1.0.0
owner: semantic-relation-network
description: 提供语义关系网络 BC 的关系实例全生命周期 REST API 接口契约，覆盖草稿管理、版本生效与下线、关系查询、历史追溯、批量导入导出与拓扑查询能力，所有接口返回 ApiResponse<T> 统一响应格式。
type: business
---

# REST API Contract: semantic-relation-network

**Protocol**: RESTful HTTP/1.1 + JSON
**Base Path**: `/api/v1/graph`
**Version**: 1.0.0

> 所有 REST API 响应统一由 foundation-core 全局切面包装为 `ApiResponse<T>` 格式，响应包含 `code`、`message`、`data`、`traceId` 四个字段。分页接口使用 `PageRequest` 入参、`PageResult<T>` 出参。

---

## Overview

本契约定义语义关系网络 BC 对外暴露的 REST 管理接口，覆盖关系实例全生命周期：草稿管理、版本生效与下线、关系查询、历史追溯、批量导入导出与拓扑查询能力。
...
- 对外查询默认仅返回生效态关系。
- `GET /api/v1/graph/admin/relations` 管理员全状态聚合查询需管理员权限，普通消费方调用返回无权限错误。
- 跨域（跨 Bundle）关系创建须符合上游导出清单与依赖规则，未授权跨域关系返回 403。
