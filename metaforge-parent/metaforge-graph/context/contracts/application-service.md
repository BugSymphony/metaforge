---
id: semantic-relation-network.application-service
protocol: Java Interface
version: 1.0.0
owner: semantic-relation-network
description: 提供语义关系网络 BC 对外暴露的 Application Service 接口契约，下游 BC 通过 Maven 依赖 metaforge-graph-api 模块后可直接进程内调用，覆盖关系草稿管理、版本生效与下线、多维查询、历史追溯、批量导入导出与拓扑管控能力。
type: business
---

# Application Service Contract: semantic-relation-network
